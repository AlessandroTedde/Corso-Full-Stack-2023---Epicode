...
*/
me.skills = ['HTML', 'CSS', "Un pochino di Javascript"];
console.log(me);

/* ESERCIZIO F
  Crea un pezzo di codice per aggiungere un nuovo elemento all'array "skills" contenuto nell'oggetto "me".
*/
me.skills.push("Microsoft SQL");
console.log(me);
...
} console.log(isThisAnEmail("cane@cane"));
...
function onlyTheYears() {
  ...
  for (let i = 0; i < movies.length; i++) {
    const moviesYear = movies[i].Year;
    yearsArray.push(moviesYear);
  }
  return yearsArray
...
function searchByTitle(string) {
  for (let i = 0; i < movies.length; i++) {
    const collection = movies[i];
    if (collection.Title.includes(string)) {                    // perche` torna solo il primo valore? Devo fare per forza una funzione freccia?
      return collection;
    }
  }
...
function addRedBackground() {
  ...
  findA.style.backgroundColor = "red";
} addRedBackground();
...
function addLi() {
  ...
  newLi.innerText = "Nuovo elemento nella lista";
